...
class PriorityQueue:
    ...
    def is_worse_than(self, position, new_list):
        ...
        thisCost= self.queue[position][0] + self.heuristic(self.queue[position][1])
    ...
    def add(self,list_with_cost): #should receive [cost_so_far, location, tool]
        ...
        if self.is_worse_than(low, push_me):        #push_me is better than low and replaces it
            self.queue[low:low]=[push_me]
        elif not self.is_worse_than(high, push_me): #push_me is worse than high and comes above it
...
class Rescuer:
    def __init__(self, lines):
        self.depth, self.target_x, self.target_y, self.cave = Solution.parse(self, lines, True)
        ...
        self.minimal_distances.append([ [-1]*(max_x+1) for y in range(max_y+1)]) #init torch
        self.minimal_distances.append([ [-1]*(max_x+1) for y in range(max_y+1)]) #init climbing
        self.minimal_distances.append([ [-1]*(max_x+1) for y in range(max_y+1)]) #init neither
    ...
    def rescue(self, x,y,mins,tool):
        ...
        while not pq.queue_is_empty():
            ...
            if (location == [self.target_x, self.target_y]): #found buddy
                ...
                if self.is_tool_valid(curr_x+1, curr_y, current_tool): #go right with same tool
                    if cost+1<self.minimal_distances[current_tool][curr_y][curr_x+1] or self.minimal_distances[current_tool][curr_y][curr_x+1]==-1:
                        ...
                        self.update_distance(curr_x+1, curr_y, current_tool, cost+1)
                
                if self.is_tool_valid(curr_x+1, curr_y, next_tool): #go right with diff tool
                    if cost+8<self.minimal_distances[next_tool][curr_y][curr_x+1] or self.minimal_distances[next_tool][curr_y][curr_x+1]==-1:
                        ...
                        self.update_distance(curr_x+1, curr_y, next_tool, cost+8)
                
                if self.is_tool_valid(curr_x, curr_y+1, current_tool): #go down with same tool
                    if cost+1<self.minimal_distances[current_tool][curr_y+1][curr_x] or self.minimal_distances[current_tool][curr_y+1][curr_x]==-1:
                        ...
                        self.update_distance(curr_x, curr_y+1, current_tool, cost+1)
                
                if self.is_tool_valid(curr_x, curr_y+1, next_tool): #go down with diff tool
                    if cost+8<self.minimal_distances[next_tool][curr_y+1][curr_x] or self.minimal_distances[next_tool][curr_y+1][curr_x]==-1:
                        ...
                        self.update_distance(curr_x, curr_y+1, next_tool, cost+8)
                
                if self.is_tool_valid(curr_x-1, curr_y, current_tool): #go left with same tool
                    if cost+1<self.minimal_distances[current_tool][curr_y][curr_x-1] or self.minimal_distances[current_tool][curr_y][curr_x-1]==-1:
                        pq.add([cost+1, [curr_x-1,curr_y], current_tool])
                        self.update_distance(curr_x-1, curr_y, current_tool, cost+1)
                
                if self.is_tool_valid(curr_x-1, curr_y, next_tool): #go left with diff tool
                    if cost+8<self.minimal_distances[next_tool][curr_y][curr_x-1] or self.minimal_distances[next_tool][curr_y][curr_x-1]==-1:
                        pq.add([cost+8, [curr_x-1,curr_y], next_tool])
                        self.update_distance(curr_x-1, curr_y, next_tool, cost+8)
                
                if self.is_tool_valid(curr_x, curr_y-1, current_tool): #go up with same tool
                    if cost+1<self.minimal_distances[current_tool][curr_y-1][curr_x] or self.minimal_distances[current_tool][curr_y-1][curr_x]==-1:
                        pq.add([cost+1, [curr_x,curr_y-1], current_tool])
                        self.update_distance(curr_x, curr_y-1, current_tool, cost+1)
