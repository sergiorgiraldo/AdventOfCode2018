...
class Group:
    def __init__(self, num_of_units, hit_points, attack_damage, attack_type, initiative, immune_to, weak_to, index=None, camp=None, boost=0):
...
class Solution(InputAsLinesSolution):
    ...
    def parse(self, lines):
        ...
        for line in lines:
            ...
            groups.append(Group(num_of_units, hit_points, attack_damage,
                          attack_type, initiative, immune_to, weak_to, index, camp=camp))
            index += 1
    ...
    def battle(self, groups, boost=0):
        ...
        while any(group.camp == 0 for group in groups) and any(group.camp == 1 for group in groups):
            # check if damage is lower than the hp and deaths stopped happening
            stale = True
            ...
            for selecting_group in sorted(groups, key=lambda g: (-g.effective_power(), -g.initiative)):
                enemies = [g for g in groups if g.camp !=
                           selecting_group.camp and g.attacked_by is None]
                if len(enemies) == 0:
                    continue
            ...
            if stale: groups = [group for group in groups if group.camp == 1] # infection wins
